# 📚 Índice de Documentación - Linguistika v2.0

Guía completa de toda la documentación disponible en el proyecto.

---

## 🚀 Inicio Rápido

### Para Comenzar
1. **[README_v2.md](../README_v2.md)** - Visión general del proyecto v2.0
2. **[guias/00_COMIENZA_AQUI.md](guias/00_COMIENZA_AQUI.md)** - Primer paso recomendado
3. **[guias/QUICKSTART.md](guias/QUICKSTART.md)** - Instalación rápida
4. **[guias/GUIA_WINDOWS.md](guias/GUIA_WINDOWS.md)** - Instalación específica para Windows

---

## 📁 Backend

### Documentación General
- **[backend/README.md](../backend/README.md)** - Documentación completa del backend

### Guías de Configuración
- **[backend/docs/guias/GUIA-SUPABASE.md](../backend/docs/guias/GUIA-SUPABASE.md)** - Configurar Supabase
- **[backend/docs/guias/CHECKLIST-SUPABASE.md](../backend/docs/guias/CHECKLIST-SUPABASE.md)** - Checklist de migración

### Migraciones SQL
- **[backend/docs/migraciones/supabase-schema.sql](../backend/docs/migraciones/supabase-schema.sql)** - Schema base
- **[migraciones/MIGRACION_SESIONES_MOVIMIENTOS.sql](migraciones/MIGRACION_SESIONES_MOVIMIENTOS.sql)** - Nuevas tablas ⭐

---

## 📖 Especificaciones Técnicas (v2.0)

- **[ESPECIFICACION_NUEVA_ESTRUCTURA.md](ESPECIFICACION_NUEVA_ESTRUCTURA.md)** - Arquitectura v2.0
- **[GUIA_IMPLEMENTACION_PAGOS.md](GUIA_IMPLEMENTACION_PAGOS.md)** - Sistema de pagos
- **[RESUMEN_CAMBIOS_2026-01-19.md](RESUMEN_CAMBIOS_2026-01-19.md)** - Changelog completo

---

**Última actualización:** 19 Enero 2026
